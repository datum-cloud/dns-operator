...
const (
	...
	// directory says "skills": the path is fixed by the capability documents
	// the assistant already ships, so it is not ours to rename.
...
// knowledgeHandler serves this operator's knowledge and skills over plain
// HTTP.
//
...
// newKnowledgeHandler reads every embedded document into memory. Failing
// here is a build problem, not a runtime one, so the server refuses to start
// rather than serve a capability document's URLs as 404s.
...
func (h *knowledgeHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// Every byte is fixed at build time and already in memory: one write of
	// a known-size buffer, no per-request I/O.
